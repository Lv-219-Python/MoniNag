...
class LoginView(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(first_name="FirstName",
    ...
    def test_already_activated(self):
        """Ensure that activate() method works properly, if user is already activated"""
    ...
    def test_register_user(self):
        ...
        expected_json = json.dumps({'error': {},
                                    'message': 'Thank you for your time. The confirmation code has been sent to '
                                    'your email. In order to confirm the registration, simply click on '
    ...
    def test_already_registered_email(self):
        """Ensure that we get an error when we try to register with an already registered email"""
        """First, we register a user"""

        reg_data = json.dumps({'email': 'test@test.test',
                               'firstName': 'TestName',
                               'lastName': 'LastName',
                               'password': 'isecretlyloveunittests'})
        ...
        response = self.client.post(url, data=reg_data, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        """Now we try to register that email once more"""
        new_reg_data = json.dumps({'email': 'test@test.test',
                                   'firstName': 'Iam',
                                   'lastName': 'Inlove',
                                   'password': 'withunittests'})

        expected_json = json.dumps({'error': 'This email is already registered. Please try another',
                                    'message': {},
                                    'success': False})
    ...
    def test_registration_invalid_email(self):
        ...
        expected_json = json.dumps({'error': "The email address you've entered has not a valid format",
                                    'message': {},
                                    'success': False})
    ...
    def test_request_password_reset(self):
        ...
        expected_json = json.dumps({'error': {},
                                    'message': 'An email has been sent to testmail@test.so. Please check its inbox to continue reseting password.',
                                    'success': True})
    ...
    def test_request_password_reset_nonexisting_user(self):
        """Ensure we can't send an email if it is not connected to some user"""
    ...
    @mock.patch('registration.views.urlsafe_base64_decode', Fakeuidb64.uidb64)
    ...
    def test_confirm_password_reset(self):
        ...
        user = CustomUser.objects.create(first_name="TestN",
                                         second_name="SecondName",
                                         email="testmail2@test.so",
                                         avatar="avatartest",
                                         activation_key="testkey",
                                         is_active=True)
        ...
        user.save()
        reg_data = json.dumps({'uidb64': '15',
                               'token': '15',
                               'password': 'testpass'})

        url = reverse('confirm_password_reset', args=['15', '15'])
        response = self.client.post(url, data=reg_data, content_type='application/json')
        expected = ({'error': {},
                     'message': "Password has been reset. In 5 seconds you will be redirected to the login page.",
                     'success': True})
    ...
    def test_confirm_password_reset_link_unavailable(self):
        """Ensure we get an error when trying to confirm the reset with unworking link"""
